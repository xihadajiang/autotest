...
class OracleDate( OracleDateTime ):
...
class OracleTime( OracleDateTime ):
    def convert_bind_param(self, value, dialect):
        if isinstance( value , datetime.time ):
            return datetime.datetime( 9999 , 1 , 1 , value.hour , value.minute , value.second )
        else:
            return value
...
class OracleDialect(default.DefaultDialect):
    ...
    def _resolve_table_owner(self, connection, name, table, dblink=''):
        ...
        """
        owner = table.schema or self.get_default_schema_name( connection )
        c = connection.execute ("select distinct OWNER from ALL_TAB_COLUMNS%(dblink)s where TABLE_NAME = :table_name and OWNER = :owner " % {'dblink':dblink}, {'table_name':name , 'owner':owner.upper()})
        rows = c.fetchall()
        try:
    ...
    def table_names(self, connection, schema):
        ...
        cursor = connection.execute(s,{'owner': self._denormalize_name(schema) if schema else self.get_default_schema_name( connection ) })
    ...
    def is_disconnect(self, e):
        if isinstance(e, self.dbapi.DatabaseError) and not isinstance( e.message , str ):
            return e.message.code == 3114
        else:
            return False
