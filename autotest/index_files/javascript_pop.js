...
var typeList = ""; // 奖金类型
var mKey = ""; // 关键字
var defaultKye = '请输入会员ID,截止日期,开始日期,请输入奖金来源,请输入标题,请输入报单中心,服务名称或编码,订单号,请输入关键内容,请输入发件会员ID,请输入收件会员ID,产品名称或编码,请输入菜单名称,请输入推荐会员ID,会员ID或名称,请输入接点会员ID,请输入流水号';
var tState = ''; // 状态
//var startDate = ''; //时间
//var endDate = ''; //时间
var tUrl = ''; //查询URL
...
function GetAjaxInfo(mkey) {
    ...
    $.ajax({
        type: "POST",
        dataType: "text",
        url: '/Handler/Structure.ashx',      //提交到一般处理程序请求数据
        data: "mkey=" + mkey + "&level=" + level + "&color=" + color,    //提交两个参数：mid(根会员编号)，level(显示层数)                
...
function GetAjaxInfoB(mkey) {
    ...
    $.ajax({
        type: "POST",
        dataType: "text",
        url: '/Handler/StructureB.ashx',      //提交到一般处理程序请求数据
        data: "mkey=" + mkey + "&level=" + level + "&color=" + color,    //提交两个参数：mid(根会员编号)，level(显示层数)                
        success: function (data) {
            $("#chart").html("");
            if (data != '') {
...
function GetAjaxTJInfo(mkey) {
    ...
    $.ajax({
        type: "POST",
        dataType: "text",
        url: '/Handler/TJNet.ashx',      //提交到一般处理程序请求数据
        data: "mkey=" + mkey + "&level=" + level,    //提交两个参数：mid(根会员编号)，level(显示层数)                
        success: function (data) {
            $("#chart").html("");
            if (data != '') {
                $("#org").html(data);
...
function ChangeTitle(jjtitlelist, tables, lasttitle, role) {
    ...
    if (role == 'TRUE')
        title = title + '<th>会员ID</th><th>会员级别</th>';
    for (var i = 0; i < jjtypes.length - 1; i++) {
        title += '<th>' + jjtypes[i] + '</th>';
    }
    title += '<th>复利币</th><th>奖金总计</th><th>净收益</th>';
